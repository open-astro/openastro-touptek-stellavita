#!/bin/bash
# OpenAstro layer for the ToupTek StellaVita (Raspberry Pi CM4 based).
#
# This turns a stock Raspberry Pi OS Lite (arm64, Trixie) image into the
# OpenAstro OS for the StellaVita: a WiFi access point (OpenAstro-XXXX /
# 12345678), baked-in credentials (astro/astro, no first-boot wizard), the
# StellaVita hardware enablement (12V power outputs, Cypress USB hub power,
# Renesas xHCI firmware), and the plumbing AlpacaBridge expects. On first
# boot the SSID gains a per-board suffix from the wlan0 MAC (e.g.
# OpenAstro-915D) so multiple boards don't collide.
#
# Hardware layer (per https://www.openastro.net/docs/sbc-install/touptek-stellavita):
#   - config.txt gpio=18,10,17,4,9,11=op,dh,pu - GPIO 18/10/17/4 switch the
#     12V DC power outputs on at boot; GPIO 9 and 11 power the Cypress USB hub.
#   - Renesas uPD72020x xHCI firmware at /lib/firmware/renesas_usb_fw.mem
#     (the StellaVita's USB ports hang off a Renesas xhci-pci controller
...
# ============================================================
# StellaVita hardware: 12V power outputs + USB hub/controller
# ============================================================
# GPIO 18, 10, 17, 4 switch the StellaVita's 12V DC power outputs; GPIO 9
# and 11 power the Cypress USB hub. Drive them all high (op,dh) with
# pull-ups (pu) from the firmware at boot, so the DC outputs and USB ports
# are live before Linux even starts - AlpacaBridge then toggles them at
# runtime via GPIO.
log "Enabling 12V power outputs and USB hub power (config.txt)..."
GPIO_LINE="gpio=18,10,17,4,9,11=op,dh,pu"
CONFIG_TXT="/boot/firmware/config.txt"
if [ -f "$CONFIG_TXT" ]; then
    grep -qxF "$GPIO_LINE" "$CONFIG_TXT" || {
        printf '\n# OpenAstro StellaVita: 12V power outputs + Cypress USB hub power\n%s\n' \
...
log "OpenAstro OS layer complete (StellaVita power/USB + WiFi AP + identity + AlpacaBridge)."
